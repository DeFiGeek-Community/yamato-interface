...
export function useSelfRedeemCallback(): {
  // signatureData: SignatureData | undefined | null  FIXME: EIP-2612
  state: CallbackState;
  callback: null | ((eth: number, expected: number) => Promise<string>);
  error: string | null;
} {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      callback: async function onSelfRedeem(
        eth: number,
        expected: number
      ): Promise<string> {
        // payload
        ...
        // estimate gas
        const call = await estimateGas('selfRedeem', value, option, signer);
        if ('error' in call) {
        ...
        try {
          ...
          throw new Error(getErrorMessage('selfRedeem', error));
        }
      },
      error: null,
    };
